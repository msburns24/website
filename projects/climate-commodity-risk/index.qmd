...
description: "El Niño pushes rubber up and cotton down. Building an early-warning system for climate-driven commodity price shocks — and finding the places it fails."
date: 2026-07-26
...
- **Problem:** A footwear manufacturer buys cotton and natural rubber from two
  dozen countries. Climate shocks move both prices, but nobody could say which
  shocks, how far ahead, or by how much — so hedging and forward-buying
  decisions were made on instinct.

- **What I built:** Three stacked layers over a 300-month panel of prices,
  ERA5 climate reanalysis, ENSO indices and disaster counts — SARIMAX and
  VAR/Granger for the causal story, gradient-boosted and logistic classifiers
  for a 3/6/9-month spike alarm, and a Monte Carlo scenario engine for
  planning ranges.

- **Key result:** The same El Niño shock moves the two commodities in
  *opposite* directions — rubber **+36.9%** at 12 months, cotton **−10.9%** —
  so any pooled "climate risk" model is wrong by construction. The alarm is
  genuinely skillful for rubber (AUC **0.775**, Brier skill **+0.182**) and
  weak for cotton. The most useful thing I found is a negative result: the
  relative alarm rule everyone reaches for first goes **silent exactly when
  the risk is highest**.
:::
...
contain: **which** driver, at **what lag**, and **how big**.

So the project was scoped to the decisions a sourcing team actually makes. Do I
forward-buy this quarter? What price goes in next year's budget? How wrong could
that number be? Those questions have different shapes, and they don't collapse
into one model. They became three layers:
...
<svg class="ccr-diagram" viewBox="0 0 820 452" role="img"
  ...
  <defs>
    <marker id="ccrArrow" viewBox="0 0 10 10" refX="9" refY="5"
            markerWidth="6" markerHeight="6" orient="auto-start-reverse">
      <path d="M 0 0 L 10 5 L 0 10 z" fill="var(--ccr-line)"/>
    </marker>
  </defs>
  ...
  <text class="ccr-label" x="32" y="80">QUESTION</text>
  <text class="ccr-body" x="32" y="100">Does climate move price &#8212;</text>
  <text class="ccr-body" x="32" y="118">which way, at what lag?</text>
  <line class="ccr-rule" x1="32" y1="136" x2="248" y2="136"/>
  <text class="ccr-label" x="32" y="160">METHOD</text>
  <text class="ccr-body" x="32" y="180">ADF &#183; Granger &#183; SARIMAX</text>
  <text class="ccr-body" x="32" y="198">VAR with orthogonalised IRFs</text>
  <line class="ccr-rule" x1="32" y1="216" x2="248" y2="216"/>
  <text class="ccr-label" x="32" y="240">OUTPUT</text>
  <text class="ccr-body" x="32" y="260">Signed elasticities and a</text>
  <text class="ccr-body" x="32" y="272">two-quarter lead</text>
  ...
  <text class="ccr-label" x="302" y="80">QUESTION</text>
  <text class="ccr-body" x="302" y="100">Is a +10% spike coming</text>
  <text class="ccr-body" x="302" y="118">within the next h months?</text>
  <line class="ccr-rule" x1="302" y1="136" x2="518" y2="136"/>
  <text class="ccr-label" x="302" y="160">METHOD</text>
  <text class="ccr-body" x="302" y="180">XGBoost &#183; regularised logistic</text>
  <text class="ccr-body" x="302" y="198">Walk-forward, OOS from 2016</text>
  <line class="ccr-rule" x1="302" y1="216" x2="518" y2="216"/>
  <text class="ccr-label" x="302" y="240">OUTPUT</text>
  <text class="ccr-body" x="302" y="260">A monthly alarm probability</text>
  <text class="ccr-body" x="302" y="272">at h = 3, 6, 9</text>
  ...
  <text class="ccr-label" x="572" y="80">QUESTION</text>
  <text class="ccr-body" x="572" y="100">What number goes in the</text>
  <text class="ccr-body" x="572" y="118">budget, and how wrong is it?</text>
  <line class="ccr-rule" x1="572" y1="136" x2="788" y2="136"/>
  <text class="ccr-label" x="572" y="160">METHOD</text>
  <text class="ccr-body" x="572" y="180">Bootstrapped residual Monte</text>
  <text class="ccr-body" x="572" y="198">Carlo &#8212; 10,000 paths, 12 months</text>
  <line class="ccr-rule" x1="572" y1="216" x2="788" y2="216"/>
  <text class="ccr-label" x="572" y="240">OUTPUT</text>
  <text class="ccr-body" x="572" y="260">Percentile planning prices</text>
  <text class="ccr-body" x="572" y="272">and P(+10% by h)</text>

  <!-- dependency: L1 -> L3 -->
  <path d="M 140 292 C 140 322, 680 322, 680 292" fill="none"
  ...
  <!-- shared panel -->
  <rect class="ccr-card" x="12" y="368" width="796" height="72" rx="8"/>
  <rect x="12" y="368" width="4" height="72" rx="2" fill="var(--ccr-l4)"/>
  <text class="ccr-label" x="36" y="396">ONE MONTHLY PANEL &#183; 300 MONTHS &#183; 2001-01 &#8594; 2025-12</text>
  <text class="ccr-body" x="36" y="420">World Bank Pink Sheet prices &#183; ERA5 anomalies &#183; NOAA CPC ONI &#183; EM-DAT disaster counts</text>

  <line class="ccr-rule" x1="140" y1="368" x2="140" y2="356" marker-end="url(#ccrArrow)"/>
  <line class="ccr-rule" x1="410" y1="368" x2="410" y2="356" marker-end="url(#ccrArrow)"/>
  <line class="ccr-rule" x1="680" y1="368" x2="680" y2="356" marker-end="url(#ccrArrow)"/>
</svg>
```

Three layers over one panel — and the dashed arrow is the part that matters.
Layer 3 is Layer 1's SARIMAX specification refit, so when a scenario shifts a
driver and the price moves, it moves *because of* Layer 1's coefficient. That is
...
**anomalies** — z-scores against each region's own climatology — so Xinjiang and
...
That caching rule is worth dwelling on, because it cost me a rebuild. Bronze
...
doesn't matter much — a confident, well-validated, completely wrong answer. So
each commodity gets its own driver set, its own lag structure, and its own
horizons, and nothing is shared but the pipeline.

There's a second asymmetry worth stating, because it changes the modelling
target. A manufacturer that *buys* these materials is structurally short both:
only *upward* moves hurt. A symmetric price forecast spends half its capacity on
the half of the distribution nobody is exposed to. That's why Layer 2 predicts
spikes rather than prices.
...
it — while log returns reject overwhelmingly (**−5.35** and **−12.84**). So
...
sign of causing El Niño, which is the sanity check you want. The SARIMAX
...
`drought_3m_lag6` is **+0.0126** (`p=0.019`) for cotton: *wetter* anomalies
raise prices six months later. Most people — I did — expect drought to spike
cotton. In Xinjiang's heavily irrigated system the mechanism appears to run the
other way, with excess rain at the wrong point in the cycle disrupting harvest
and quality. The temperature term keeps its negative sign (`t2m_anom_lag6`
**−0.0028**) but at `p=0.325` it carries no weight, and I don't give it any.
...
label is a maximum over the window, not the end-to-end change, because a spike that
happens and reverses still hits a buyer who had to purchase during it.
...
cotton 19% → 41%, rubber 28% → **56%**. Rubber's h=9 skill is real, but a +10%
move within nine months is close to rubber's normal state, so the *value* of
knowing is lower than 0.775 suggests. That gap between skill and usefulness
turns out to be the whole story of the next section.
...
**1.25× the historical base rate** rather than a fixed cutoff — "spike risk is
elevated 25% above normal." At that operating point, cotton h=3 crosses at
τ=0.24 and fires **54 alarms against 22 real events**, catching 13 with 41 false
alarms. Rubber h=6 crosses at τ=0.56: **55 alarms, 51 events, 34 hits, 21
false** — a 67% hit rate.
...
70%. The skill is real and the alarm is nearly mute.
...
- **Rubber's replay ranked the episode well.** Episode AUC **0.786**, peaking at
  **0.820** in November 2010. It never fired because the pre-2010 base rate was
  69%, putting the 1.25× bar at **0.858** — and the peak missed it by **0.038**.
...
different windows — so that agreement would mean something. But **they are not
equally independent, and it would be easy to over-claim.** Layer 3's engine *is*
the Layer 1 SARIMAX spec, refit. When a scenario shifts a driver and the price
moves, it moves *because of* Layer 1's coefficient. That's arithmetic, not
confirmation. Only Layer 2 is genuinely independent: different model class,
...
- **"Wet is bullish for cotton" is corroborated, but weakly.** The alarm model
  ...
  mean |SHAP| **0.057**. That is genuine cross-method agreement rather than one
...
**No raw data ships with the repo.** Each source carries its own licence, and
...
backtest at your own spend. No server, no login — the whole payload is 383 KB
of static CSV.
:::
...
This began as a graduate capstone project at UNC's School of Data Science and
Society, with teammates who built the Streamlit application and the
country-level data exploration. It is an independent academic project, not affiliated with or
endorsed by Nike.
:::
