...
subtitle: "A practitioner's account of building a production-ready classifier on 130 million Amazon reviews — and everything a Jupyter notebook doesn't teach you."
date: 2026-05-25
...
```
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│ classifier   │───▶│  fallback    │───▶│    drift     │
│ DistilBERT   │    │ three-tier   │    │ OOV / length │
│ + validation │    │ recovery     │    │ / label dist │
└──────────────┘    └──────────────┘    └──────────────┘
       │                                        │
       ▼                                        ▼
┌──────────────┐                    ┌──────────────────┐
...
Persistent artifacts:
  predictions.jsonl    ← every API call logged
  human_queue.jsonl    ← tier-3 escalations, append-only
  reports/drift.json   ← drift simulation output
```
...
```
              ...
              └──────────────┬───────────────┘
                             ...
                             ▼
              ┌──────────────────────────────┐
              │   TIER 1: DistilBERT Model   │
              │   confidence ≥ 0.60 → label  │
              └──────────────┬───────────────┘
                   ok ◄──────┤ confidence < 0.60
                             ▼
              ┌──────────────────────────────┐
...
Every path through this diagram produces the same response shape:
...
The `status` field tells the caller which tier responded. `"ok"` means the model was confident. `"fallback"` means a lower tier handled it. `"rejected"` means the input failed validation. The caller never needs to handle an exception — the system's contract is that it always returns this shape.

This design principle — *never raise to the caller* — is the single most important production pattern in the codebase. It means downstream systems can treat the classifier as a reliable function rather than a fragile service that needs try/except wrappers throughout the call chain.

The keyword tier (Tier 2) isn't sophisticated. It matches three keyword sets — 18 positive terms like "excellent" and "wonderful," 18 negative terms like "awful" and "scam," and 11 neutral terms like "mediocre" and "acceptable" — then picks the class with the most hits. It handles the cases that fall through Tier 1 where the model was uncertain but the review contains clear signal words. It's fast, it's auditable, and it keeps the human queue from filling up with easy cases. If two classes tie, or if no keywords match at all, the request escalates rather than guessing.

The human queue (Tier 3) is an append-only JSONL file. Every routed review gets a timestamp and a reason code. This file feeds the observability dashboard and, eventually, the retraining pipeline — because human-labeled edge cases are more valuable training data than any synthetic augmentation.
...
The OOV signal held — the late-window vocabulary isn't significantly different from what DistilBERT saw during training. If OOV rate were the only signal monitored, this drift event would have been missed entirely.

The text length and label distribution signals are unambiguous. Reviews in the late window are systematically longer (the KS p-value is effectively zero — 10⁻⁵⁸ is not a rounding issue). And the sentiment balance has shifted dramatically: positive reviews fell from 34% to 13% while negative reviews surged from 61% to 83%. That's a review-bombing pattern — a period where negative sentiment concentrates and the volume of detailed, lengthy complaints increases.

The practical consequence: a model trained on the early window's balanced distribution and evaluated against the late window's heavily skewed one will see F1 regression on the positive class, which it was trained to expect at roughly 3× the frequency it now encounters. The drift report doesn't fix this — but it fires before the model's accuracy metrics have time to visibly degrade, which is when the fix is cheapest.

The actionable thresholds (OOV > 5%, KS p-value < 0.05, JS divergence > 0.05) are all operator-configurable. The simulation script accepts `--oov-threshold`, `--length-pvalue`, and `--js-threshold` flags so sensitivity can be tuned without touching code. Different business contexts warrant different sensitivities: a fraud detection system might want hair-trigger drift alerts; a product review classifier might tolerate more variance before escalating.

```bash
python scripts/simulate_drift.py --output-json reports/drift.json
```

The JSON output feeds directly into the dashboard's drift report panel.
...
*The full codebase, including the FastAPI serving layer, drift simulation script, Streamlit observability dashboard, and four-layer test suite, is on [GitHub](https://github.com/msb/amazon-sentiment-ml-eng).*
